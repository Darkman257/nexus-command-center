...
async function inspectPatch() {
  ...
  try {
    ...
    await writeFile(join(finalDir, 'apply_plan.md'), `# Apply Plan\n- Requires OWNER APPROVAL.\n- Auto Apply is FORBIDDEN.\n- To apply, manually copy files from ${join(finalDir, 'patch_data', 'changed_files')} to destination project.`);
    
    console.log(`INSPECTION COMPLETE. Status: ${status}. Files moved to ${finalDir}`);
